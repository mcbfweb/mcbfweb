			...
			.widget(
					...
					$.mobile.widget,
					{
						...
						refresh : function(create) {
							...
							if (!create) {
								this.element.find('.ui-inlinetabs-content')
										.unwrap();
							}

							var $element = this.element, $tabbar = $element
									.children(':not(:jqmData(tab-ignore))')
									.first(), $tabs = $tabbar
									.children(':not(:jqmData(tab-ignore))'), $content = $element
									.children(':gt(0):not(:jqmData(tab-ignore))'), $container = $content
									.wrapAll(
											'<div class="ui-inlinetabs-container" />')
									.parent(), o = this.options, $activeTab = $tabs
									.filter(
											create ? ":jqmData(active)" : "."
													+ o.activeClass).first();
							...
							$tabs
									.each(function() {
										...
										if (create
												|| !$this.hasClass('ui-inlinetabs-tab')) {
$this.buttonMarkup({
																corners : false,
																shadow : false,
																inline : true,
																...
																iconpos : $this
																		.jqmData('icon') ? ($this
																		.jqmData('iconpos') || o.iconpos)
																		: undefined,
																mini : o.mini
															})
													.addClass(
															'ui-corner-all ui-inlinetabs-tab');
										}
									});
							...
							// Tabbed Content
							var $activeContent = $activeTab.jqmData('tab') ? $content
									.filter(":jqmData(tab='"
											+ $activeTab.jqmData('tab') + "')")
									: null;

							if (o.contentTheme) {
								$container.addClass('ui-corner-all ui-body-'
										+ o.contentTheme);

								if (o.theme) {
									$container
											.addClass('ui-inlinetabs-container-top');
								}
							}

							$content.addClass('ui-inlinetabs-content').hide();
							if ($activeContent) {
								$activeContent.show();
							}
							...
							if (!create) {
								$tabbar.off("vclick.inlinetabs",
										".ui-inlinetabs-tab");
							}

							// change tabs on click/tap event
							$tabbar.on("vclick.inlinetabs",
									".ui-inlinetabs-tab", function(event) {
							...
							if (create && o.scroll) {
								$tabbar
										...
										.on(
												"vmousemove",
												function(e) {
													var $this = $(this), x = e.pageX, originalX = $this
															.jqmData('scrollpos');
													...
													var newScroll = $this
															.scrollLeft()
															- x + originalX, maxScroll = $this[0].scrollWidth
															- $element.width();
						...
						show : function(tab) {
							this.element.find(
									".ui-inlinetabs-tab:jqmData(tab='" + tab
											+ "')").trigger('vclick');
						}
					});
